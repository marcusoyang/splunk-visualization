...
class btctocurrency(GeneratingCommand):
    ...
    def get_currency_exchange(self, currency):
        ...
        return data["rates"][currency.lower()]["value"]

    def generate(self):
        exchange_rate = self.get_currency_exchange(self.currency)
        yield {'_time': time.time(), 'coin': self.currency.lower(), 'exchange_rate': exchange_rate, '_raw': str(self.currency) + "," + str(exchange_rate)}
